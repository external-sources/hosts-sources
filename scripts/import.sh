...
printf "Downloading Active hosts-file.net.... from https://bitbucket.org/expiredsources/hosts-file.net/src/master/\n"
	${WGET} -qO- "https://bitbucket.org/expiredsources/hosts-file.net/raw/master/active/ad_servers.txt" | awk '/^(#|$)/{ next }; { if ( $2 ~ /[a-z]/ ) printf("%s\n",tolower($2)) | "sort -i | uniq -u -i " }' | perl -lpe 's/^\s*(.*\S)\s*$/$1/' > "data/hphosts_ad_servers/domain.list"
	${WGET} -qO- "https://bitbucket.org/expiredsources/hosts-file.net/raw/master/active/emd.txt" | awk '/^(#|$)/{ next }; { if ( $2 ~ /[a-z]/ ) printf("%s\n",tolower($2)) | "sort -i | uniq -u -i " }' | perl -lpe 's/^\s*(.*\S)\s*$/$1/' > "data/hphosts_emd/domain.list"
	${WGET} -qO- "https://bitbucket.org/expiredsources/hosts-file.net/raw/master/active/exp.txt" | awk '/^(#|$)/{ next }; { if ( $2 ~ /[a-z]/ ) printf("%s\n",tolower($2)) | "sort -i | uniq -u -i " }' | perl -lpe 's/^\s*(.*\S)\s*$/$1/' > "data/hphosts_exp/domain.list"
	${WGET} -qO- "https://bitbucket.org/expiredsources/hosts-file.net/raw/master/active/fsa.txt" | awk '/^(#|$)/{ next }; { if ( $2 ~ /[a-z]/ ) printf("%s\n",tolower($2)) | "sort -i | uniq -u -i " }' | perl -lpe 's/^\s*(.*\S)\s*$/$1/' > "data/hphosts_fsa/domain.list"
	${WGET} -qO- "https://bitbucket.org/expiredsources/hosts-file.net/raw/master/active/grm.txt" | awk '/^(#|$)/{ next }; { if ( $2 ~ /[a-z]/ ) printf("%s\n",tolower($2)) | "sort -i | uniq -u -i " }' | perl -lpe 's/^\s*(.*\S)\s*$/$1/' > "data/hphosts_grm/domain.list"
	${WGET} -qO- "https://bitbucket.org/expiredsources/hosts-file.net/raw/master/active/hjk.txt" | awk '/^(#|$)/{ next }; { if ( $2 ~ /[a-z]/ ) printf("%s\n",tolower($2)) | "sort -i | uniq -u -i " }' | perl -lpe 's/^\s*(.*\S)\s*$/$1/' > "data/hphosts_hjk/domain.list"
	${WGET} -qO- "https://bitbucket.org/expiredsources/hosts-file.net/raw/master/active/mmt.txt" | awk '/^(#|$)/{ next }; { if ( $2 ~ /[a-z]/ ) printf("%s\n",tolower($2)) | "sort -i | uniq -u -i " }' | perl -lpe 's/^\s*(.*\S)\s*$/$1/' > "data/hphosts_mmt/domain.list"
	${WGET} -qO- "https://bitbucket.org/expiredsources/hosts-file.net/raw/master/active/psh.txt" | awk '/^(#|$)/{ next }; { if ( $2 ~ /[a-z]/ ) printf("%s\n",tolower($2)) | "sort -i | uniq -u -i " }' | perl -lpe 's/^\s*(.*\S)\s*$/$1/' > "data/hphosts_psh/domain.list"
	${WGET} -qO- "https://bitbucket.org/expiredsources/hosts-file.net/raw/master/active/pup.txt" | awk '/^(#|$)/{ next }; { if ( $2 ~ /[a-z]/ ) printf("%s\n",tolower($2)) | "sort -i | uniq -u -i " }' | perl -lpe 's/^\s*(.*\S)\s*$/$1/' > "data/hphosts_pup/domain.list"
